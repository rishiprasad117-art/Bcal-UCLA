...
# --- CONFIGURATION ---
BASE_URL = "https://dining.ucla.edu/de-neve-dining/"
DOMAIN = "https://dining.ucla.edu"
LOCATION_NAME = "De Neve"

# Stations to specifically IGNORE
EXCLUDED_STATIONS = [
    "Field Greens Bar",
    "Salad Bar",
    "Frozen Yogurt"
...
_OUTPUT_FILE = "data/raw/ucla_de_neve_nutrition_oz.csv"
...
def get_menu_links():
    """Scrapes the De Neve menu page. Filters out any items belonging to excluded stations."""
    ...
    for elem in elements:
        ...
        # 2. Detect Station
        if elem.name in ['h3', 'h4', 'h5'] and len(text) < 60 and "See Meal Details" not in text:
            current_station = text

        # --- EXCLUSION CHECK ---
        if any(excl.lower() in current_station.lower() for excl in EXCLUDED_STATIONS):
            continue
        ...
        if elem.name == 'a' and 'See Meal Details' in text:
            ...
            if parent:
                ...
                if "Field Greens Bar" in food_name or "Field Greens Bar" in current_station:
                    continue
    ...
    print(f"Found {len(menu_items)} items (excluding excluded stations).")
    return menu_items
