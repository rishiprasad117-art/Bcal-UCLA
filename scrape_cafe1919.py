...
# --- CONFIGURATION ---
BASE_URL = "https://dining.ucla.edu/cafe-1919/"
DOMAIN = "https://dining.ucla.edu"
LOCATION_NAME = "Cafe 1919"

_MEAL_KEYWORDS = {"BREAKFAST", "LUNCH", "DINNER", "EXTENDED DINNER", "LUNCH / DINNER", "ALL DAY"}

_OUTPUT_FILE = "data/raw/ucla_cafe_1919_nutrition.csv"
...
def get_menu_links():
    ...
    for elem in elements:
        ...
        # Cafe 1919 often uses "Lunch / Dinner" or "All Day"
        if text.upper() in _MEAL_KEYWORDS:
            current_meal = text.title()
            active_meals.add(current_meal)
            continue

        # 2. Detect Station (Headers)
        if elem.name in ['h3', 'h4', 'h5'] and len(text) < 50 and "See Meal Details" not in text:
